...
__all__ = ['EMA', 'SimAM', 'SpatialGroupEnhance', 'BiLevelRoutingAttention', 'BiLevelRoutingAttention_nchw', 'TripletAttention', 
           ...
           'LocalWindowAttention', 'ELA', 'CAA', 'CDDA']
...
class MLCA(nn.Module):
    ...
    def forward(self, x):
        ...
        temp_local = local_arv.view(b, c_local, -1).transpose(-1, -2).contiguous().reshape(b, 1, -1)
...
class CDDA(nn.Module):
    def __init__(self, in_channels, reduction=4):
        super(CDDA, self).__init__()
        inter_channels = in_channels // reduction
        self.context_pre = Conv(in_channels, inter_channels, k=1) # 使用1x1 Conv进行通道降维
        # 使用不同扩张率的空洞分组卷积
        self.context_d3 = nn.Conv2d(inter_channels, inter_channels, 3, padding=3, dilation=3, groups=inter_channels, bias=False)
        self.context_d5 = nn.Conv2d(inter_channels, inter_channels, 3, padding=5, dilation=5, groups=inter_channels, bias=False)
        self.context_bn = nn.BatchNorm2d(inter_channels)
        self.context_post = Conv(inter_channels, in_channels, k=1) # 使用1x1 Conv恢复通道

        # --- 2. 细节分支 (Detail Branch) ---
        # 负责进行高保真度的局部特征提取
        self.detail_pre = Conv(in_channels, inter_channels, k=1)
        # 使用标准的深度可分离卷积
        self.detail_dw = DWConv(inter_channels, in_channels, k=3)

        # --- 3. 注意力融合 ---
        # 直接调用 attention.py 中已有的、经典的 SEAttention 模块
        self.se_fusion = SEAttention(channel=in_channels, reduction=reduction * 2)

    def forward(self, x):
        identity = x

        # --- 上下文分支计算 ---
        c_pre = self.context_pre(x)
        c_d3 = self.context_d3(c_pre)
        c_d5 = self.context_d5(c_pre)
        F_context = self.context_post(self.context_bn(c_d3 + c_d5))

        # --- 细节分支计算 ---
        d_pre = self.detail_pre(x)
        F_detail = self.detail_dw(d_pre)
        
        # --- 初步融合 ---
        F_fused = F_context + F_detail
        
        # --- 调用SE模块进行通道注意力筛选 ---
        F_attended = self.se_fusion(F_fused)
        
        # --- 残差连接 ---
        F_out = identity + F_attended
        return F_out