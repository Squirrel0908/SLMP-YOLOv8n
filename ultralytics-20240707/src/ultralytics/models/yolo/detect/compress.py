...
from ultralytics.nn.extra_modules import Detect_LADH
from ultralytics.nn.extra_modules.attention import CDDA
...
def get_pruner(opt, model, example_inputs):
    ...
    #for yolov8-SLMP2.yaml
    for k, m in model.named_modules():
        if isinstance(m, Detect_LADH):
            ignored_layers.append(m.cv2[0][4])
            ignored_layers.append(m.cv2[1][4])
            ignored_layers.append(m.cv2[2][4])
