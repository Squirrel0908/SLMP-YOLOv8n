...
print(f"总共有 {len(image_files)} 张图片需要处理")
print(f"将为每张图片随机选择一种增强方式，最终总共有 {len(image_files) * 2} 张图像")
...
print("所有图片处理完成")
print(f"输出目录中共有 {len(os.listdir(output_image_dir))} 张图像")
print(f"输出目录中共有 {len(os.listdir(output_label_dir))} 个标签文件")
